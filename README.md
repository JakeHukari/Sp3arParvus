# Sp3arParvus - Universal Aim Client and other developer tools

Sp3arParvus provides the user a powerful ingame menu which enables esp, player tracking, and movement/physics prediction for combat as well as assistive tools like wallbreak, fullbright, preformance monitoring, and others.


### Loadstring
```lua
loadstring(game:HttpGet("https://raw.githubusercontent.com/JakeHukari/Sp3arParvus/refs/heads/main/Sp3arParvus_v2.lua", true))() 
```

This script works universally on any roblox game that uses regular player models.

### Current Features

> SilentAim.

> TriggerBot.

> Player-ESP.

> [Any-Item-ESP](https://github.com/JakeHukari/Any-Item-ESP). `Ctrl + E`

> Free-cam. `Ctrl + P`

> Waypoint System. `Ctrl + Middle-Mouse-Button` to create `Ctrl + Middle-Mouse-Button` (on an existing waypoint) to remove 

> Br3ak3r. (Wallbreak) `Ctrl + Left-Click` to break | `Ctrl + Z` to undo 

> Fullbright.

> Preformance Monitor.

> Other stuff.

### Upcoming Features

> Extended 'fullbright' customization. (Time, sky hue, reflections, colors, etc..(basically just customization for any 'light' or sky))

> Extended workspace explorer. (dex)

> Block 'placing' tool. (probably will function like wallbreak except placing a local object that you can walk on or climb up or something idk, obviously will only work locally)  


**A lot of the aim stuff is based off [Parvus Hub](https://github.com/AlexR32/Parvus) from AlexR32**

Special thanks to @[AlexR32](https://github.com/AlexR32), original author of Parvus

**For educational and development purposes only 😉**
